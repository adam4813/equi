...
function sidlLoaded() {
  ...
  parseXML("EQUI_Animations", "EQUI_Animations");
  addToFileTree("EQUI_Templates");
  parseXML("EQUI_Templates", "EQUI_Templates");
  addToFileTree("EQUI_PlayerWindow");
  parseXML("EQUI_PlayerWindow", "EQUI_PlayerWindow");
  addToFileTree("EQUI_BigBankWnd");
  parseXML("EQUI_BigBankWnd", "EQUI_BigBankWnd");
}

function parseXML(name, filename) {
  $.get("xml/" + filename + ".xml", "xml").done(function(xml) {
    ...
    if (
      schema.attributes.item(0).value == "EverQuestData" &&
      schema.attributes.item(1).value == "EverQuestDataTypes"
    ) {
    ...
    while (next != null) {
      ...
      files[name].elements[current.attributes.item(0).value] = items[
        current.attributes.item(0).value
      ] = newItem;
...
function addToFileTree(name) {
  var div = $(document.createElement("div")).attr("id", name);
  $(div)
    .addClass("inlineChildren header")
    .click({ name: name }, function(data) {
      var self = $(this);
      if (data.target == this) {
        self.find("> :first").toggle();
      }
      if (!self.data("populated")) {
        self.data("screenElement", populateFileElements(data.data.name));
        self.data("populated", true);
        self.find("> :first").show();
      }
      if (self.data("screenElement") !== undefined) {
        targetDiv = viewers["Screen"](
          items[self.data("screenElement")],
          "#renderView"
        );
        $("#renderView")
          .empty()
    ...
    .text(name)
    .append($(document.createElement("div")).attr("id", name + "ItemList"));
  $("#fileTree").append($(div));
}
...
function populateFileElements(filename) {
  ...
  for (var type in itemsByType) {
    var typeDiv = $("<div />")
      .text(type)
      .addClass("fileElementsContainer");
    var itemDiv = $("<div />")
      .hide()
      .addClass("fileElementsList");
    typeDiv.append($(itemDiv)).click(function(data) {
      if (data.target == this) {
        $(this)
          .find("> :first")
          .toggle();
...
function showElementProperties(item) {
  ...
  for (var key in items[item].elements) {
    var div = $(document.createElement("div"))
      .attr("id", item + key)
      .click(
        { key: key, value: items[item].elements[key], target: targetDiv },
        function(data) {
          if (
            data.data.value.type.search("Template") > -1 &&
            !data.data.value.isItem
          ) {
            editors["Template"](
              data.data.key,
              data.data.value,
...
function parseNode(node, parent) {
  ...
  if (name === "TextureInfo") {
    let img = viewers["TextureInfo"](
      { item: node.attributes[0].nodeValue },
      "#preload"
    );
    $("#preload").append(img);
  }
